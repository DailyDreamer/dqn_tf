...
class QNetwork(object):
  def __init__(self, agent_history_length, screen_height, screen_width, action_size, learning_rate, name):
    ...
    with tf.variable_scope(name):
      ...
      self.q, self.var['q_w'], self.var['q_b'] = linear(self.fc, action_size, activation_fn=None, name='q_func')

      self.actions = tf.argmax(self.q, 1, name="actions")
      self.q_max = tf.reduce_max(self.q, 1, name="q_max")
